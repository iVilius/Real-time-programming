...
func secondary(i int, sendChannel chan int, receiveChannel chan int, compChannel chan int, intChannel chan int) {
	...
	var value_received int  = -1
	var value_compare int = -1
	interrupt := <- intChannel 
	fmt.Println("Line 113")
	data := <- receiveChannel
	go UDP_receive("24568", receiveChannel, intChannel)
	compChannel <- data
	if (interrupt == 0) {
		go primary(i, sendChannel, intChannel)
	} else {
		for {
			value_received = <- receiveChannel
			
			if (value_received == value_compare) {
				var counter int = 0
				for j := 0; j < 5; j++ {
					value_received = <- receiveChannel
					if (value_received == value_compare) {
						counter++
						value_compare = <- receiveChannel
					} else {
						break
					}
				}
